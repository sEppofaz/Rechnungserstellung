...
| POST | `/kargl/api/adressen/{row}` | Adresszeile aktualisieren (inkl. Anrede) |
| POST | `/kargl/api/adressen/{row}/loeschen` | Adresszeile löschen |
| POST | `/kargl/api/adressen/update-by-name` | Adresse per Name suchen + überschreiben (für Adress-Diff-Pop-up) |
...
- **Template-Tabellenstruktur (ab 2026-06-12):** `{{ beschreibungstext }}` liegt in einer eigenen Zeile mit 3 normalen Zellen (5783+1985+1984=9752 Twips) – NUR Zelle 1 hat Inhalt, Zellen 2+3 leer. Kein gridSpan/Colspan mehr! Tabellenbreite 9752 Twips = nutzbare Seitenbreite (A4 minus Ränder 2×1077). Backup: `VORL_Rechnungsformular 2026_BACKUP2.dotx`. Bei Template-Änderungen in Word: keine colspan-Zeilen verwenden, alle Zeilen müssen 3 Zellen haben. Rechnungsnummer-Platzhalter: `{{ rechnungsnummer }}` (war fälschlich Festtext `xxx0026`)
...
- **`_Adressen.xlsx` Schema (ab 2026-06-12):** **Anrede** | Name | Straße | PLZ | Ort | Straße validiert | PLZ+Ort validiert | Hinzugefügt. Anrede ist Spalte A (Index 0). `find_in_address_excel()` matcht auf row[1] (Name). `add_to_address_excel()` schreibt [anrede, name, strasse, plz, ort, ...]. Bestehende Zeilen ohne Anrede → leere Spalte A → Fallback im Formular greift.
